...
public class OpenGlTextureRenderer : IDisposable
{
    ...
    private OpenGLDeviceBuffer<VertexPositionColorTexture> _vertexBuffer;
    private OpenGLDeviceBuffer<uint> _indexBuffer;
    private OpenGLShader _shader;

    private uint _currentTextureId;

    public OpenGlTextureRenderer(OpenGLGraphicsDevice device)
    {

        _loader = new EmbeddedResourceLoader(typeof(Application).Assembly, "Panthera2D.res.");
